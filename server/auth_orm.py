...
class Auth(base):
    '''
        ...
        * ``user_name`` - String of maxiumum length 80 and the primary key
    ...
    def _compute_hash(self, password):
        ...
        '''
        return hashlib.md5( (self.user_name + '|' + self.pass_salt + '|' + password).encode() ).hexdigest() # string encoded into bytes then md5
    
    def _set_hash(self, password):
        '''
            Given known ``user_name`` and ``pass_salt``, compute and store ``pass_hash``. Used for registering new users. Does not commit.
        '''
